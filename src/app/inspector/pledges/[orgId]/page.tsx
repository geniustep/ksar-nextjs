...
export default function InspectorOrgPledgesDetailPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await inspectorApi.getOrgPledges(orgId, { page, limit });
  ...
  return (
    <DashboardLayout>
      <div className="mb-4 sm:mb-6">
        <Link href="/inspector/pledges" className="text-sm text-primary-600 hover:text-primary-700 mb-2 inline-block">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="sm:hidden space-y-3">
            {items.map((item) => (
              <Link key={item.assignment_id} href={`/inspector/requests/${item.request_id}`} className="block">
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.assignment_id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      ...
                      <td className="py-3 px-3 text-center">
                        <Link href={`/inspector/requests/${item.request_id}`} className="text-xs bg-primary-50 text-primary-700 hover:bg-primary-100 px-3 py-1.5 rounded-lg transition-colors">
                          تفاصيل
                        </Link>
                      </td>
                    </tr>
